...
from typing import Optional, Tuple, Callable, Set, List
...
class Block(arcade.Sprite):
    def __init__(self, block_type: BlockType, scale=BLOCK_SCALE):
...
class GridMap(arcade.Sprite):
    ...
    def get_rc(self, row: int, col: int) -> Block:
        ...
        return self.block_list[row * self.col + col]  # noqa
...
class GameManager:
    def __init__(self):
        ...
        for block in self.map.block_list:
            block.on_animation_end = self.on_animate_end  # noqa
        self.animating_blocks: int = 0
    ...
    def clicked_block(self, block: Block):
    ...
    def get_mergeable(self) -> Set[Tuple[Block, Tuple[int, int]]]:
        mergeable: Set[Tuple[Block, Tuple[int, int]]] = set()
        for row in range(self.map.row - 2):
            for col in range(self.map.col):
                if (
                    ...
                    mergeable.add((self.map.get_rc(row, col), (row, col)))
                    mergeable.add((self.map.get_rc(row + 1, col), (row + 1, col)))
                    mergeable.add((self.map.get_rc(row + 2, col), (row + 2, col)))
        ...
        for row in range(self.map.row):
            for col in range(self.map.col - 2):
                if (
                    ...
                    mergeable.add((self.map.get_rc(row, col), (row, col)))
                    mergeable.add((self.map.get_rc(row, col + 1), (row, col + 1)))
                    mergeable.add((self.map.get_rc(row, col + 2), (row, col + 2)))
        return mergeable

    def check_game_over(self): ...
    # todo 无可交换检测

    def swap_with_animation(self, b1: Block, b2: Block):
        # todo 交换后无法消除需要重置
        ...
        self.animating_blocks += 2
        self.map.block_list.swap(idx1, idx2)

    def fall_down_with_animation(self, mergeable: Set[Tuple[Block, Tuple[int, int]]]):
        # 统计每列需要消除几个，即生成几个
        count_list = [0 for _ in range(self.map.col)]
        merge_pos = [t for _, t in mergeable]
        for t in merge_pos:
            count_list[t[1]] += 1
        ...
        for col in range(self.map.col):
            ...
            first_empty_row = -1
            for row in range(self.map.row):
                if first_empty_row == -1 and (row, col) in merge_pos:
                    first_empty_row = row
                    continue
                if first_empty_row != -1 and (row, col) not in merge_pos:
            ...
            for i in range(count_list[col]):
                ...
                block.on_animation_end = self.on_animate_end
            ...
            for block in will_fall_list:
                ...
                self.animating_blocks += 1
                first_empty_row += 1

    def on_animate_end(self, block: Block):
        """block动画结束的回调函数"""
        self.animating_blocks -= 1
